...
With 81 explanatory variables describing every aspect of residential homes in Ames, Iowa, the data sets have quite many missing and outliers in the datasets. I spent a lot of time understanding all the features and replacing the missing values somewhat. For the outliers, I decided to keep them as the data number is not very large <2500. The removal of outliers will significantly affect the model, and none of the outliers are identified as caused by entry/measuring errors.
 
Further, I also exploratory the relationships of every variable to the target variable "SalePrice," which was identified to have a massive impact on SalePrice like Total Bsmt Area, Year built, Overall Qual. I also could figure out how a single variable will affect the SalePrice, like a house with Excellent Kitchen Quality tend to have better SalePrice and home with Neighborhood in Northridge, Stony Brook, and Northridge Heights tend to have high sales price.

However, when bringing all the variables together, what is the possible sale price for a house with a Neighborhood in Northridge but low kitchen Quality? There is no clue for such questions. Thus, I developed a model to predict the price of dealing with such a situation.

During the Feature engineering, I used two methods to select the features for modeling. The first method is to manually choose the features by evaluating their correlation coefficients with SalesPrice and manually combine/drop the multi-collinear variables. I use the lasso model as a feature-pre-screening tool in the second method. Three models are used for each technique: linear regression, Ridge, and lasso. In method 1, lasso gave the best performance among the three models. In method 2, Ridge gave the best performance among the three models.
